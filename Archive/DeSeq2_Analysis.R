#################################################################################################
#################################################################################################
#### Analysis of RNA-Seq data for Wildfire Toxicity Project, focusing on gene expression levels
####
#### Using DESeq2 for sample normalization and statistical analysis to identify genes associated with wildfire smoke exposure
#### Code includes: data organization, covariate imputation, PCA, surrogate variable analysis, and other steps prior to DESeq2
#### 
#### 
#### Code drafted by Lauren Eaves, Lauren Koval, Julia Rager, and Alexis Payton
#### Lasted updated: November 2, 2020
#################################################################################################
#################################################################################################
...
#only interested in comparing mouse heart peat flame (MH113-MH118) vs. mouse heart saline samples (MH101-MH106)
subsetted_colnames = substr(colnames(countdata),1,5)
samples_of_interest = c('Gene', 'MH113', 'MH114', 'MH115', 'MH116', 'MH117', 'MH118','MH101',
                                           'MH102', 'MH103', 'MH104', 'MH105','MH106') 
countdata_subset = countdata[,(substr(colnames(countdata),1,5) %in% samples_of_interest)]

dim(countdata_subset) #13 samples (plus Gene ID column), 30146 genes
...
dim(subjectinfo) #170 samples(rows), 7 col
...
##########
##########
# (1) Filtering to first keep biosmass exposure ids present in both files
##########
##########

# Pull all biomass IDs (FullID) that overlap between countdata_subset & subjectinfo
keep_samples <- colnames(countdata_subset[, colnames(countdata_subset) %in% subjectinfo$FullID]) 
# Note that in this case, this represents 12 IDs
...
countdata_subset_temp <- countdata_subset[, colnames(countdata_subset) %in% keep_samples] # pulling countdata_subset, without gene identifier col
countdata_subset <- cbind(countdata_subset[,1], countdata_subset_temp)    # adding the gene identifier column back in
colnames(countdata_subset)[1] <- "Gene"   # renaming the gene identifer column
countdata_subset_temp <- NULL    # removing temporary dataframe
...
# This resulted in filtering the original list of 30,146 genes down to 16,067 genes
...
# Pull all biomass IDs that overlap between countdata_subset & subjectinfo
keep_samples <- colnames(countdata_subset[, colnames(countdata_subset) %in% subjectinfo$FullID]) 
...
names(coldata)[names(coldata) == 'FullID'] <- 'ID'
...
pca_df <- data.frame(PC1 = pca$x[,1], PC2 = pca$x[,2], PC3 = pca$x[,3], Sample=colnames(countdata_subset))
...
countdata_forclustering <- t(countdata_subset)
countdata_forclustering[1:5,1:10]


# run heirarchical clustering
hc <-hclust(dist(countdata_forclustering))
jpeg(filename= paste0(Output,"/",cur_date, "_heirachicalclusteringforoutliers.jpeg"),width = 4500, height = 350)
plot(hc)
dev.off()

# Didn't do these next steps because there weren't any outliers
...
write.csv(countdata_subset, paste0(Output,"/", cur_date, "_RawCounts_SubjectsIncludedinModel.csv"), row.names= TRUE)
write.csv(coldata, paste0(Output,"/", cur_date, "_SubjectInfo_SubjectsIncludedinModel.csv"), row.names= FALSE)
...
sapply(countdata_subset, class)   # checking the class of the values
countdata_subset_2 <- as.data.frame(sapply(countdata_subset, as.integer))   # converting into integers if showing numeric
sapply(countdata_subset_2, class)   # re-checking the class of the values
row.names(countdata_subset_2) <- row.names(countdata_subset)  # this removed the row names (gene identifiers), so had to merge these back
countdata_subset <- countdata_subset_2
...
# In this case, we want FullTreatment = 'Saline' (first level, aka, the "control" level), and FullTreatment = 'PeatFlame', cases as numerators
dds$FullTreatment <- relevel (factor(dds$FullTreatment), "Saline") 
...
write.csv(normcounts, paste0(Output,"/", cur_date, "_NormCounts_IncludedSubjects.csv"), row.names=TRUE)

# pseudocounts to make plots easier:
ps_normcounts <- normcounts + 1
write.csv(ps_normcounts, paste0(Output,"/",cur_date, "_NormCounts_ps_IncludedSubjects.csv"),row.names=TRUE)

# log2 pseudocounts (y=log2(n+1))
log2normcounts <- log2(normcounts+1)
write.csv(log2normcounts, paste0(Output,"/", cur_date, "_NormCounts_pslog2_IncludedSubjects.csv"), row.names=TRUE)
...
write.csv(vsd_matrix, paste0(Output,"/", cur_date, "_VSDCounts_IncludedSubjects.csv"), row.names=TRUE)
...
# This ran a Wald test p-value (Full Treatment PeatFlame vs. Saline)
...
write.csv(as.data.frame(res)[order(res$padj),], paste0(Output,"/", cur_date,"_AllStatResults.csv"))


#################################################################################################
#################################################################################################
#### Overview of statistical results (This is where I stopped, so I haven't done any plotting yet.)
#################################################################################################
#################################################################################################
summary(res)
