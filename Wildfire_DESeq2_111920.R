...
dev.copy(png,paste0(Output,"/", cur_date,"_PCA_PlateBatch.png"))
dev.off()
...
ggplot(pca_df, aes(PC1,PC2, color = Treatment))+
  geom_point(size=1) +
  labs(x=paste0("PC1 (",pca_percent[1],"%)"), y=paste0("PC2 (",pca_percent[2],"%)"))
dev.copy(png,paste0(Output,"/", cur_date,"_PCA_Treatment.png"))
dev.off()
...
ggplot(pca_df, aes(PC1,PC2, color = Tissue))+
  geom_point(size=1) +
  labs(x=paste0("PC1 (",pca_percent[1],"%)"), y=paste0("PC2 (",pca_percent[2],"%)"))
dev.copy(png,paste0(Output,"/", cur_date,"_PCA_Tissue.png"))
dev.off()
...
ggplot(pca_df, aes(PC1,PC2))+
  ...
  labs(x=paste0("PC1 (",pca_percent[1],"%)"), y=paste0("PC2 (",pca_percent[2],"%)"))
dev.copy(png,paste0(Output,"/", cur_date,"_PCA_IDsLabeled.png"))
dev.off()
...
jpeg(filename= paste0(Output,"/", cur_date, "_Hierachical_clustering_outliers.jpeg"),width = 4500, height = 350)
plot(hc)
dev.off()
...
#coldata$Treatment_Tissue <- as.factor(coldata$Treatment_Tissue)
